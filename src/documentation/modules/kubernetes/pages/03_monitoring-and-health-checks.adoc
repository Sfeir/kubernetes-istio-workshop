...
Explore the `manifests/app/pods/healthy-monolith.yaml` pod configuration file in the Cloud Shell editor

Notice `livenessProbe` and `readinessProbe` container's configuration.


Now create the `healthy-monolith` pod using kubectl:
```
kubectl apply -f manifests/app/pods/healthy-monolith.yaml
```
...
> The `healthy-monolith` Pod logs each health check. Use `./stern heal*` command to view them.
...
You may also monitor pods status in the Workloads view of GKE console (click on the *REFRESH* button on the top of the Workloads list).
...
xref:kubernetes:04_managing-application-configurations-and-secrets.adoc[Next >]