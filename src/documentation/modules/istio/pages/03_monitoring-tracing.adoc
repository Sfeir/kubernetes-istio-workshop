...
.Before Start
====
The following dashboards are not directly accessible, we need to use a proxy by using ``kubectl prot-forward``.

To do that on Cloud Shell, we'll use the *Web Preview* feature.

To use Web Preview, click the Web Preview button and then select the port number from the displayed menu. Cloud Shell opens the preview URL on its proxy service in a new browser window.

You can simultaneously preview four other consoles via Web Preview proxied on ranges 8080 to 8084.
====
...
[source,bash]
----
kubectl apply -f manifests/istio/recommendation_requestcount.yml -n istio-system
----
